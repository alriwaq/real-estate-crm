<template>
  ...
  <div v-if="project.doc" ref="parentRef" class="flex h-full">
    <Resizer
      v-if="project.doc"
              ...
              <div class="flex gap-4 items-center">
                <div class="group relative h-15.5 w-15.5">
                  <Avatar
                    size="3xl"
                    class="h-15.5 w-15.5"
                    :label="project.doc.project_name"
                    :image="project.doc.project_logo"
                  />
                  <component
                    :is="project.doc.project_logo ? Dropdown : 'div'"
                    v-bind="
                      project.doc.project_logo
                        ? {
                            options: [
                              {
                                icon: 'upload',
                                label: project.doc.project_logo
                                  ? __('Change Image')
                                  : __('Upload Image'),
                ...
                <div class="flex flex-col gap-2 truncate">
                  <div class="truncate text-2xl font-medium text-ink-gray-9">
                    <span>{{ project.doc.name }}</span>
                  </div>
                  <div
                    v-if="project.doc.website"
                    class="flex items-center gap-1.5 text-base text-ink-gray-8"
                  >
                    <WebsiteIcon class="size-4" />
                    <span>{{ website(project.doc.website) }}</span>
                  </div>
...
onMounted(async () => {
  if (project.doc) await triggerOnRender()
})
...
const title = computed(() => {
  ...
  return project.doc?.[t] || props.projectId
})
...
function changeOrganizationImage(file) {
  project.setValue.submit({
    project_logo: file?.file_url || null,
  })
}

function beforeFieldChange(data) {
  if (Object.hasOwn(data ?? {}, 'project_name')) {
    call('frappe.client.rename_doc', {
      doctype: 'Real Estate Project',
      old_name: props.projectId,
      new_name: data.project_name,
    }).then(() => {
      router.push({
        name: 'Organization',
        params: { projectId: data.project_name },
      })
    })
  } else {
    project.save.submit()
...
function openWebsite() {
  if (!project.doc.website) {
    toast.error(__('No Website Found'))
    return
  }

  openExternalWebsite(project.doc.website)
}
...
const contacts = createListResource({
  type: 'list',
  doctype: 'Contact',
  cache: ['contacts', props.projectId],
  fields: [
    'name',
    'full_name',
    'image',
  ...
  async (_doc) => {
    if (scripts.data?.length) {
      let s = await setupCustomizations(scripts.data, {
        ...
        router,
        toast,
        updateField: project.setValue.submit,
        createToast: toast.create,
        deleteDoc: deleteOrganization,
        call,
      })
      project._actions = s.actions || []
    }
  },
  { once: true },
)
