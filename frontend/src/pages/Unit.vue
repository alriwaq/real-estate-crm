<template>
  ...
  <div v-if="unit.doc" ref="parentRef" class="flex h-full">
    <Resizer
      v-if="unit.doc"
              ...
              <div class="flex gap-4 items-center">
                <div class="group relative h-15.5 w-15.5">
                  <Avatar
                    size="3xl"
                    class="h-15.5 w-15.5"
                    :label="unit.doc.unit_name"
                    :image="unit.doc.unit_logo"
                  />
                  <component
                    :is="unit.doc.unit_logo ? Dropdown : 'div'"
                    v-bind="
                      unit.doc.unit_logo
                        ? {
                            options: [
                ...
                <div class="flex flex-col gap-2 truncate">
                  <div class="truncate text-2xl font-medium text-ink-gray-9">
                    <span>{{ unit.doc.name }}</span>
                  </div>
                  <div
                    v-if="unit.doc.website"
                    class="flex items-center gap-1.5 text-base text-ink-gray-8"
                  >
                    <WebsiteIcon class="size-4" />
                    <span>{{ website(unit.doc.website) }}</span>
                  </div>
...
const {
  ...
  scripts,
  triggerOnRender,
} = useDocument('Real Estate Unit', props.unitId)

const canDelete = computed(() => permissions.data?.permissions?.delete || false)

onMounted(async () => {
  if (unit.doc) await triggerOnRender()
})
...
const title = computed(() => {
  ...
  return unit.doc?.[t] || props.unitId
})
...
function changeOrganizationImage(file) {
  unit.setValue.submit({
    unit_logo: file?.file_url || null,
  })
}

function beforeFieldChange(data) {
  if (Object.hasOwn(data ?? {}, 'unit_name')) {
    call('frappe.client.rename_doc', {
      doctype: 'Real Estate Unit',
      old_name: props.unitId,
      new_name: data.unit_name,
    }).then(() => {
      router.push({
        name: 'Organization',
        params: { unitId: data.unit_name },
      })
    })
  } else {
    unit.save.submit()
...
function openWebsite() {
  ...
  openExternalWebsite(unit.doc.website)
}
  ...
  async (_doc) => {
    if (scripts.data?.length) {
      let s = await setupCustomizations(scripts.data, {
        ...
        router,
        toast,
        updateField: unit.setValue.submit,
        createToast: toast.create,
        deleteDoc: deleteOrganization,
        call,
      })
      unit._actions = s.actions || []
    }
  },
  { once: true },
)
